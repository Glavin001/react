...
describe("ReactTestRendererTraversal", () => {
  ...
  it("searches via .findByProps() / .findAllByProps()", () => {
    ...
    expect(render.root.findAllByProps({ foo }, { deep: false })).toHaveLength(
      1
    );
    expect(render.root.findAllByProps({ bar }, { deep: false })).toHaveLength(
      5
    );
    expect(render.root.findAllByProps({ baz }, { deep: false })).toHaveLength(
      2
    );
